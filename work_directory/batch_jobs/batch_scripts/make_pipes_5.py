...
#It is an upgade of make_pipes_4.py.
...
#instead of calculating them. This is to test how much slowdown is due to the loops in this part of the code. Any temporary files created are hardcoded with the suffix '_SpeedTempTest' and resulting pipes are redirected to the subdirectory /u/cwalker/Illustris_Zhang_Method/SpeedTempTest/ so they do not get mixed up with originals during testing while simultaneously creating data for the same sim and snapshot with older code versions.
#The difference between this and make_pipes_4.py is it also stores the density, coordinate, internal energy and electron
#abundance fields for every cell chosen to lie along the sightline through the pipeline to aid in WHIM calculations.
...
def process_sim_chunk(snap_number,basePath,sim_to_use,nSubLoads,chunkIDs,T_h,T_c,c1s,c2e):
    ...
    for i in range(len(chunkIDs)): #loop over all chunk ids
        ...
        #CHARLIE NOTE (08/06/2022):
        #currently hard-coded with _SpeedTempTest suffix which should be removed in final faster version
        part_outdata_filename = '/u/cwalker/Illustris_Zhang_Method/temp_chunks/sim_{0:02d}_snap_{1:03d}_cID_{2}_SpeedTempTest.npy'.format(snap_number,chunkID,sim_to_use) 
        ...
        pipe_cell_ie_part = data['InternalEnergy'][inidx] #internal energy in erg/g
        ...
        temp_dict['InternalEnergy']    = pipe_cell_ie_part
...
nSubLoads = 1000 #number of subloads to split simulation into #chaanged from 100 -> 200 ->300 ->400 -> 600 -> 800->1000 on 26/04/22 while testing efficient parallel processing also doubled number of nodes being used to 14 from 7. change back if this doesn't generate pipes.
...
for snapshot_to_process in range(len(snaps_to_process)):
    ...
    #CHARLIE NOTE (08/06/2022):
    #currently hard-coded with _SpeedTempTest subdir and suffix which should be removed in final faster version
    outfile_name = '/u/cwalker/Illustris_Zhang_Method/SpeedTempTest/Sim_{0}_Snap_{1}_dDMdz_Output_SpeedTempTest.npy'.format(sim_to_use,snap_number) #name of this file (in separate speed directory)
    ...
    if existcheck == False:
        ...
        #edit 08/06/22 for storing whim information along sightline
        dict_to_edit['LoSBinCoords']=[] #the coordinates of the bins along the pipe line of sight
        dict_to_edit['LoSCellCoords'] = [] #the true coordinates of the cells along the pipe line of sight
        dict_to_edit['LoSDensities'] = [] #the densities of the cells along the pipe line of sight
        dict_to_edit['LoSInternalEnergies'] = [] #the internal energies of the cells along the pipe line of sight
        dict_to_edit['LoSElAbs'] = [] #the electron abundances of the cells along the pipe line of sight
        dict_to_edit['LoSPartIDs'] = [] #the particle id numbers of the cells along the pipe line of sight
    ...
    ###########################
        ...
        while(len(dict_to_edit['dDMdz_Pakmor'])<npipes): #while not enough pipes have been created:
            ...
            all_ie    = [] #particle internal energy
            ...
            for i in range(nSubLoads): #loop over parts
                ...
                #CHARLIE NOTE (08/06/2022):
                #currently hard-coded with _SpeedTempTest suffix which should be removed in final faster version
                toload_filename = '/u/cwalker/Illustris_Zhang_Method/temp_chunks/sim_{0:02d}_snap_{1:03d}_cID_{2}_SpeedTempTest.npy'.format(snap_number,i,sim_to_use) 
                ...
                all_ie.append(loaded_dict['InternalEnergy'])
            ...
            pipe_cell_ie   = np.array([item for sublist in all_ie for item in sublist])
            ...
            pipe_cell_ie_z    = pipe_cell_ie[np.where(pipe_cell_sfr==0)]
            ...
            pipe_nearest_ie_p     = np.array(pipe_cell_ie_p[nearest_idxs_p])     #internal energies [erg/g]
            ...
            pipe_nearest_ie_z     = np.array(pipe_cell_ie_z[nearest_idxs_z])     #internal energies [erg/g]
            ...
            #edit 08/06/22 for storing los density and temperature information for whim analysis and to be able to reproduce 
            #subhalo id and impact info properly
            dict_to_edit['LoSBinCoords'].append(pipe_bin_coords) #the coordinates of the bins along the pipe line of sight
            dict_to_edit['LoSPartIDs'].append(pipe_nearest_pIDs_p) #the particle id numbers of the cells along the pipe los
            dict_to_edit['LoSCellCoords'].append(pipe_nearest_coords_p) #the true coordinates of the binned cells along the pipe line of sight
            dict_to_edit['LoSDensities'].append(pipe_nearest_dens_p) #the densities of the cells along the pipe line of sight
            dict_to_edit['LoSInternalEnergies'].append(pipe_nearest_ie_p) #the internal energies of the cells along the pipe line of sight
            dict_to_edit['LoSElAbs'].append(pipe_nearest_elab_p) #the electron abundances of the cells along the pipe line of sight
